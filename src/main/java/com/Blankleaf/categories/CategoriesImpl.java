...
public class CategoriesImpl implements CategoriesDAO{
	...
	public Categories getCategory(String name) {
		// TODO Auto-generated method stub
		return this.sessionFactory.getCurrentSession().createQuery("from Categories where name=:name").setString("name", name).list().size() == 0 ? null : 
			(Categories)this.sessionFactory.getCurrentSession().createQuery("from Categories where name=:name").setString("name", name).list().get(0);
}
